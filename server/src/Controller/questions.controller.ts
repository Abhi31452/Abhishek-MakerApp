...
export async function getAllQuestionswithID(req: Request, res: Response): Promise<Response> {
  try {
    const questionPaperId = req.params._id;
    console.log(questionPaperId);
    
    const allquestion = await Question.find({ questionPaperId: questionPaperId });
